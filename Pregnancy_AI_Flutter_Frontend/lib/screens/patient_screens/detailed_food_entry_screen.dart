...
class _DetailedFoodEntryScreenState extends State<DetailedFoodEntryScreen> {
  ...
  // bool _isLoading = false; // no longer used
  ...
  Future<void> _fetchPregnancyInfo() async {
    final vm = context.read<DetailedFoodEntryProvider>();
    await vm.fetchPregnancyWeek(widget.userId);
    setState(() {
      _pregnancyWeek = vm.pregnancyWeek ?? _pregnancyWeek;
      _isLoadingPregnancyInfo = vm.isLoadingWeek;
    });
    if (!vm.isLoadingWeek && vm.error == null) {
  ...
  Future<void> _saveDetailedFoodEntry() async {
    ...
    final vm = context.read<DetailedFoodEntryProvider>();
    final data = await vm.saveDetailedFoodEntry(foodData);
    if (data['success'] == true) {
      ...
      final err = data['error']?.toString() ?? 'Unknown error saving detailed food entry';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
