...
class _MedicationDosageListScreenState
  ...
  Future<void> _loadUpcomingDosages() async {
    try {
      ...
      print('Error loading upcoming dosages: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Consumer<GetUpcomingDosagesProvider>(
        builder: (context, upcomingProvider, child) {
          if (upcomingProvider.isLoading) {
            return const Center(child: CircularProgressIndicator());
          }

          if (upcomingProvider.errorMessage != null) {
            return Center(
              child: Column(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (upcomingProvider.upcomingDosages.isNotEmpty) ...[
                    _buildSectionHeader(
                      'Upcoming Dosages',
                      upcomingProvider.totalUpcoming,
                      Icons.schedule,
                      Colors.blue,
                    ),
                  ...
                  if (upcomingProvider.prescriptionMedications.isNotEmpty) ...[
                    _buildSectionHeader(
                      'Prescription Medications',
                      upcomingProvider.totalPrescriptions,
                      Icons.medication,
                      Colors.green,
                    ),
                    const SizedBox(height: 8),
                    ...upcomingProvider.prescriptionMedications
                        .map((med) => _buildPrescriptionCard(med)),
                    const SizedBox(height: 24),
                  ],

                  // Empty State
                  if (upcomingProvider.upcomingDosages.isEmpty &&
                      upcomingProvider.prescriptionMedications.isEmpty)
                    Center(
                      child: Column(
                        children: [
                          Icon(
