...
class _DoctorAppointmentsScreenState extends State<DoctorAppointmentsScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      // Load appointments (typed) and patients in parallel
      final results = await Future.wait([
        _appointmentsRepository.getAppointmentsAsModels(),
        _appointmentsService.getDoctorPatients(),
      ]);

      final List<AppointmentModel> appointmentModels = results[0] as List<AppointmentModel>;
      final Map<String, dynamic> patientsResponse = results[1] as Map<String, dynamic>;

      List<Map<String, dynamic>> appointments = appointmentModels.map(_appointmentModelToMap).toList();
  ...
  Map<String, dynamic> _appointmentModelToMap(AppointmentModel m) {
    return {
      'appointment_id': m.id,
      'patient_id': m.patientId,
      'patient_name': m.patientName,
      'appointment_date': m.appointmentDate,
      'appointment_time': m.appointmentTime,
      'appointment_type': m.appointmentType ?? 'General',
      'appointment_status': m.status ?? 'scheduled',
      'notes': m.notes ?? '',
  ...
  Future<void> _deleteAppointment(String appointmentId) async {
    ...
    if (confirmed == true) {
      try {
        final response = await _appointmentsService.deleteAppointment(appointmentId);
