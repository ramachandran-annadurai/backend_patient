...
class _DoctorPatientDetailScreenState extends State<DoctorPatientDetailScreen> {
  ...
  final GetMedicationHistoryService _medHistoryService = GetMedicationHistoryService();
  final GetSymptomAnalysisReportsService _symptomReportsService = GetSymptomAnalysisReportsService();
  final GetGPT4AnalysisHistoryService _gpt4HistoryService = GetGPT4AnalysisHistoryService();
  final GetTabletTrackingHistoryService _tabletTrackingService = GetTabletTrackingHistoryService();
  final GetPrescriptionDetailsService _prescriptionDetailsService = GetPrescriptionDetailsService();
  final GetKickHistoryService _kickHistoryService = GetKickHistoryService();
  ...
  Future<void> _loadPatientDetails() async {
    ...
    try {
      ...
      final results = await Future.wait(<Future<Map<String, dynamic>>>[
        _appointmentsService.getPatientDetails(widget.patientId),
        _appointmentsService.getAppointments(patientId: widget.patientId),
        _medHistoryService.call(widget.patientId),
        _symptomReportsService.call(widget.patientId),
        _gpt4HistoryService.call(widget.patientId),
        _tabletTrackingService.call(widget.patientId),
        _prescriptionDetailsService.call(widget.patientId),
        _kickHistoryService.call(widget.patientId),
      ]);
  ...
  Widget _buildMentalHealthLogs() {
    final mentalHealthLogs = List<Map<String, dynamic>>.from(_patientData!['mental_health_logs'] ?? []);
  ...
  Widget _buildMedicationHistory() {
    return GestureDetector(
      onTap: () => _navigateToMedicationHistory(),
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: Colors.white,
  ...
  Widget _buildFoodNutritionLogs() {
    return GestureDetector(
      onTap: () => _navigateToFoodNutrition(),
      child: Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
  ...
  Widget _buildSymptomReports() {
    return GestureDetector(
      onTap: () => _navigateToSymptomReports(),
      child: Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
