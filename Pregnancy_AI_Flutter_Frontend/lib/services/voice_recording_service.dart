...
class VoiceRecordingService {
  ...
  // AudioRecorder removed temporarily to fix compilation issues
  ...
  Future<bool> startRecording() async {
    try {
      ...
      if (kIsWeb) {
        ...
        print('📱 Mobile voice recording placeholder...');
        print('🔄 Voice recording on mobile is currently being implemented');
        print('💡 For now, please use text input or try the web version');
        
        // Simulate recording start for UI consistency
        _isRecording = true;
  ...
  Future<String?> stopRecording() async {
    try {
      ...
      _isRecording = false; // Set this first to prevent multiple calls
        ...
        print('📱 Stopping mobile recording placeholder...');
        _isRecording = false;
        
        // Simulate a successful recording stop for UI consistency
        await Future.delayed(Duration(milliseconds: 300));
        print('✅ Mobile recording stopped (placeholder)');
        
        return 'mobile_voice_placeholder';
      }
    } catch (e) {
  ...
  Future<void> forceStopRecording() async {
    try {
      ...
      if (kIsWeb) {
        ...
        print('📱 Force stopping mobile recorder (placeholder)...');
        _isRecording = false;
        print('✅ Mobile force stop completed');
      }
  ...
  // Dispose resources
  void dispose() {
    // AudioRecorder disposed temporarily removed
  ...
  // Get recording status - simplified placeholder
  Stream<String> get recordingState {
    // Return a simple stream that just reports the current state
    return Stream.periodic(const Duration(milliseconds: 100), (_) {
      return _isRecording ? 'recording' : 'stopped';
    });
  }

  // Get amplitude level for visualization - simplified placeholder
  ...
  Future<String?> transcribeAudio(String audioFilePath) async {
    try {
      ...
      if (kIsWeb) {
        ...
        // Mobile platform - placeholder implementation
        print('📱 Mobile transcription placeholder...');
        if (audioFilePath == 'mobile_voice_placeholder') {
          return 'Voice recording completed on mobile. Please type your food details manually or use the web version for voice transcription.';
        }
        
        // For actual files (when record plugin is re-added)
